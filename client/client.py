...
async def handle_streaming(client: A2AClient, text: str, task_id: str | None = None, context_id: str | None = None):
    ...
    # FIX: Added the 'id' field, which is required by the SendStreamingMessageRequest model.
    request = SendStreamingMessageRequest(
        id=uuid4().hex,
        params=MessageSendParams(**build_message_payload(text, task_id, context_id))
    )
    ...
    async for update in client.send_message_streaming(request):
        ...
        if hasattr(update.root, "result"):
            ...
            if hasattr(result, "taskId"): # Check for taskId to update it
...
async def interactive_loop(client: A2AClient, supports_streaming: bool):
    ...
    while True:
        ...
        try:
            ...
            rprint(f"[red bold]❌ Protocol Error:[/red bold] The agent claims to support streaming but failed to provide a valid stream.")
            rprint(f"[red]   Details: {e}[/red]")
            rprint(f"[yellow]   This is likely an issue with the agent's implementation. Check the agent's server logs.[/yellow]")
        except Exception as e:
            rprint(f"[red bold]An unexpected error occurred:[/red bold] {e}")
            traceback.print_exc()
# -----------------------------------------------------------------------------
